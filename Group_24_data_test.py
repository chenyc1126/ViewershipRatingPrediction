import pandas as pd
training_source_events = pd.read_csv("test_source_events.csv")
training_source_events_columns = training_source_events.columns
new_training_source_events = pd.DataFrame()
#new_raining_source_events = pd.read_csv("new_training_source_events.csv")
...
for i in range(len(training_source_events["event_time"])):
    time = (training_source_events["event_time"][i] - 1630256400) % 604800
    day = time // 86400
    hr = (time % 86400) // 3600
    if 0 <= hr <= 7:
        slot = day*4
...
#存檔成csv
new_training_source_events.to_csv("new_test_source_events.csv")


#收集同個使用者同個timeslot的index
import pandas as pd
new_training_source_events = pd.read_csv("new_test_source_events.csv")
...
final_training_source_events.to_csv("final_test_source_events.csv")
